...
public class WidgetContainer extends AbstractContainerMenu {
	public static ResourceLocation SLOTGROUP_PLAYER = PipeGoggles.resource("player_slots");
	...
	private boolean guiDirty = false;
	...
	// We are relying on the client to tell the server which slots are currently goggleMode,
	// see MessageEnabledSlots.
	@Override
	public ItemStack quickMoveStack(Player playerIn, int index) {
		Slot uncastSlot = this.slots.get(index);
		if(uncastSlot == null || !uncastSlot.hasItem() || !(uncastSlot instanceof WidgetSlot slot)) {
